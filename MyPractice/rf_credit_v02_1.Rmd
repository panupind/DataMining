...
*Original code did not work as is *
*This is the modified file from what is uploaded in LMS, where all the character variables have been changed to factors*
```{r}
...
creditdata$CHECK_A = as.factor(creditdata$CHECK_A)
creditdata$C_HIST = as.factor(creditdata$C_HIST)
creditdata$PURPOSE = as.factor(creditdata$PURPOSE)
creditdata$SAVE_A = as.factor(creditdata$SAVE_A)
creditdata$EMPLOY = as.factor(creditdata$EMPLOY)
creditdata$PERSONAL = as.factor(creditdata$PERSONAL)
creditdata$GUARANTEE = as.factor(creditdata$GUARANTEE)
creditdata$PROPERTY = as.factor(creditdata$PROPERTY)
creditdata$INSTALL_P = as.factor(creditdata$INSTALL_P)
creditdata$HOUSING = as.factor(creditdata$HOUSING)
creditdata$JOB = as.factor(creditdata$JOB)
creditdata$TEL = as.factor(creditdata$TEL)
creditdata$FOREIGN = as.factor(creditdata$FOREIGN)
str(creditdata)
```
...
rf_gridsearch <- train(CREDIT~., data=traindata, method="rf", metric=metric, tuneGrid=tunegrid, trControl=control, ntree=50)
print(rf_gridsearch)
plot(rf_gridsearch)
```
...
RF2<- randomForest(CREDIT ~ ., data = traindata , 
                   ntree=50, mtry = 15 ,importance=TRUE )
RF2
...
RF3<- randomForest(CREDIT ~ ., data = traindata, 
                   ntree=500, mtry = 15,importance=TRUE, 
                   cutoff=c(0.7,0.3) )
RF3
```
